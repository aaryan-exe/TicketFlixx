...
namespace CineGo
{
	public partial class WebForm6 : System.Web.UI.Page
	{
        ...
        {
            MarkBookedSeats(); // Always refresh seat status
        ...
        private void MarkBookedSeats()
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("SELECT SeatID FROM BatmanTbl WHERE Status = 'Booked'", conn))
                {
        ...
        protected void Seat_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("UPDATE BatmanTbl SET Status = 'Booked' WHERE SeatID = @SeatID", conn))
                {
